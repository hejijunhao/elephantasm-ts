...
export class Elephantasm {
   ...
   */
  async inject(options: InjectOptions = {}): Promise<MemoryPackWithHelpers> {
    ...
    const pack = await this.request<MemoryPack>(path);
  ...
  private wrapMemoryPack(pack: MemoryPack): MemoryPackWithHelpers {
    return {
      ...pack,
      asPrompt(): string {
        return pack.content.context || '';
      },
      getIdentity(): IdentityContext | undefined {
        return pack.content.identity;
      },
      getSessionMemories(): ScoredMemory[] {
        return pack.content.session_memories || [];
      },
      getKnowledge(): ScoredKnowledge[] {
        return pack.content.knowledge || [];
      },
      getLongTermMemories(): ScoredMemory[] {
        return pack.content.long_term_memories || [];
      },
      getTemporalContext(): TemporalContext | undefined {
        return pack.content.temporal_context;
      },
    };
  }
}
